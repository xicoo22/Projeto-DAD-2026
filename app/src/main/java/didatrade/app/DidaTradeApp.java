...
public class DidaTradeApp {
	...
	private boolean populate(int quantity) {
		...
		GenericResponseCollector<DidaTradeMain.PopulateReply> populate_collector = new GenericResponseCollector<DidaTradeMain.PopulateReply>(
				populate_responses, this.n_servers);

		for (int i = 0; i < n_servers; i++) {
			CollectorStreamObserver<DidaTradeMain.PopulateReply> populate_observer = new CollectorStreamObserver<DidaTradeMain.PopulateReply>(
					populate_collector);
	...
	private boolean adduser(int uid, int cash, int stock) {
		...
		GenericResponseCollector<DidaTradeMain.AddUserReply> adduser_collector = new GenericResponseCollector<DidaTradeMain.AddUserReply>(
				adduser_responses, this.n_servers);

		for (int i = 0; i < n_servers; i++) {
			CollectorStreamObserver<DidaTradeMain.AddUserReply> adduser_observer = new CollectorStreamObserver<DidaTradeMain.AddUserReply>(
					adduser_collector);
	...
	private boolean sell(int uid, int quantity) {
		...
		GenericResponseCollector<DidaTradeMain.SellReply> sell_collector = new GenericResponseCollector<DidaTradeMain.SellReply>(
				sell_responses, this.n_servers);

		for (int i = 0; i < n_servers; i++) {
			CollectorStreamObserver<DidaTradeMain.SellReply> sell_observer = new CollectorStreamObserver<DidaTradeMain.SellReply>(
					sell_collector);
	...
	private boolean buy(int uid, int quantity) {
		...
		GenericResponseCollector<DidaTradeMain.BuyReply> buy_collector = new GenericResponseCollector<DidaTradeMain.BuyReply>(
				buy_responses, this.n_servers);

		for (int i = 0; i < n_servers; i++) {
			CollectorStreamObserver<DidaTradeMain.BuyReply> buy_observer = new CollectorStreamObserver<DidaTradeMain.BuyReply>(
					buy_collector);
	...
	private int balance(int uid) {
		...
		GenericResponseCollector<DidaTradeMain.BalanceReply> balance_collector = new GenericResponseCollector<DidaTradeMain.BalanceReply>(
				balance_responses, this.n_servers);

		for (int i = 0; i < n_servers; i++) {
			CollectorStreamObserver<DidaTradeMain.BalanceReply> balance_observer = new CollectorStreamObserver<DidaTradeMain.BalanceReply>(
					balance_collector);
	...
	private boolean show() {
		...
		GenericResponseCollector<DidaTradeMain.DumpReply> dump_collector = new GenericResponseCollector<DidaTradeMain.DumpReply>(
				dump_responses, n_servers);

		for (int i = 0; i < n_servers; i++) {
			CollectorStreamObserver<DidaTradeMain.DumpReply> dump_observer = new CollectorStreamObserver<DidaTradeMain.DumpReply>(
					dump_collector);

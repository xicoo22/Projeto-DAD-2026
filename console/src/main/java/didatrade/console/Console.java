					...
					if ((parameter1 != null) && (parameter2 != null)) {
						...
						if (ballot_number < this.getLastBallot())
							...
							try {
								...
								DidaTradeMaster.NewBallotRequest.Builder newballot_request = DidaTradeMaster.NewBallotRequest
										.newBuilder();
								ArrayList<DidaTradeMaster.NewBallotReply> newballot_responses = new ArrayList<DidaTradeMaster.NewBallotReply>();
								;
								GenericResponseCollector<DidaTradeMaster.NewBallotReply> newballot_collector = new GenericResponseCollector<DidaTradeMaster.NewBallotReply>(
										newballot_responses, 1);
								CollectorStreamObserver<DidaTradeMaster.NewBallotReply> newballot_observer = new CollectorStreamObserver<DidaTradeMaster.NewBallotReply>(
										newballot_collector);
								...
								new Thread(new Runnable() {
									public void run() {
										...
										if (newballot_responses.size() >= 1) {
											...
											System.out.println("reply received to new ballot request for ballot = " + ballot_number
					...
					if (parameter1 != null) {
						...
						if (ballot_number < this.getLastBallot())
							...
							try {
								...
								;
								GenericResponseCollector<DidaTradeMaster.ActivateReply> activate_collector = new GenericResponseCollector<DidaTradeMaster.ActivateReply>(
										activate_responses, 1);
								CollectorStreamObserver<DidaTradeMaster.ActivateReply> activate_observer = new CollectorStreamObserver<DidaTradeMaster.ActivateReply>(
										activate_collector);
								...
								new Thread(new Runnable() {
									public void run() {
										...
										if (activate_responses.size() >= 1) {
											...
											System.out.println("reply received to activate request for ballot = " + ballot_number
													+ " with ack = " + activate_reply.getAck());
										} else
				...
				case "debug":
					...
					if ((parameter1 != null) && (parameter2 != null)) {
						try {
							...
							;
							GenericResponseCollector<DidaTradeMaster.SetDebugReply> setdebug_collector = new GenericResponseCollector<DidaTradeMaster.SetDebugReply>(
									setdebug_responses, 1);
							CollectorStreamObserver<DidaTradeMaster.SetDebugReply> setdebug_observer = new CollectorStreamObserver<DidaTradeMaster.SetDebugReply>(
									setdebug_collector);

...
public class MainLoop implements Runnable {
	...
	public synchronized void processEntry(int entry_number) {
		...
		while (next_entry.decided == false) {
			...
			System.out.println("QUORUM = " + quorum + " for ballot = " + ballot);
			...
			if ((ballot > -1) && (request_record != null)
				...
				PhaseOneResponseProcessor phase_one_processor = new PhaseOneResponseProcessor(this.server_state.scheduler,
						low_ballot, high_ballot, quorum, n_acceptors);
				// PhaseOneBogusProcessor phase_one_processor = new
				// PhaseOneBogusProcessor(this.server_state.scheduler, low_ballot, high_ballot);

				ArrayList<DidaTradePaxos.PhaseOneReply> phase_one_responses = new ArrayList<DidaTradePaxos.PhaseOneReply>();
				GenericResponseCollector<DidaTradePaxos.PhaseOneReply> phase_one_collector = new GenericResponseCollector<DidaTradePaxos.PhaseOneReply>(
						phase_one_responses, n_acceptors, phase_one_processor);

				for (int i = 0; i < n_acceptors; i++) {
					CollectorStreamObserver<DidaTradePaxos.PhaseOneReply> phase_one_observer = new CollectorStreamObserver<DidaTradePaxos.PhaseOneReply>(
							phase_one_collector);
				...
				if (ballot_aborted == false) {
					...
					GenericResponseCollector<DidaTradePaxos.PhaseTwoReply> phase_two_collector = new GenericResponseCollector<DidaTradePaxos.PhaseTwoReply>(
							phase_two_responses, n_acceptors, phase_two_processor);
					for (int i = 0; i < n_acceptors; i++) {
						CollectorStreamObserver<DidaTradePaxos.PhaseTwoReply> phase_two_observer = new CollectorStreamObserver<DidaTradePaxos.PhaseTwoReply>(
								phase_two_collector);
		...
		switch (action) {
			...
			case DidaTradeAction.ADDUSER:
				result = this.server_state.trade_manager.add_user(command.getUserId(), command.getQuantity(),
						command.getStock());
				break;
